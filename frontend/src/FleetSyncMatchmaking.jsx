...
//Esse menu era para aparecer em cima do menu cinza
function Menu({ searchQuery, setSearchQuery, onOpenNotifications, onOpenProfile }) {
  return (
    <nav className="top-0 left-0 right-0 h-16 bg-zinc-950 border-b border-zinc-800 px-6 flex items-center justify-between z-40 shadow-lg">
...
function SubMenu({ searchQuery, setSearchQuery, onOpenNotifications, onOpenProfile }) {
  return (
...
export default function FleetSyncMatchmaking() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-800 font-sans antialiased p-6 max-w-7xl mx-auto w-full space-y-6">
      
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 pb-4">
        <div>
          <h2 className="text-xl font-black text-zinc-900 tracking-tight flex items-center gap-2">
            <Zap className="w-5 h-5 fill-amber-400 stroke-zinc-950 stroke-[1.5]" /> Matchmaking Inteligente de Cargas
          </h2>
      ...
      <div className="border-t border-zinc-200 pt-6">
        <div className="mb-4">
          ...
          <p className="text-xs text-zinc-400">Status atualizado de todas as ordens de serviço</p>
        </div>
                ...
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => handleOpenOrderDetail(order)} className="text-zinc-900 hover:text-amber-500 font-bold inline-flex items-center gap-0.5 cursor-pointer">
            ...
            <div className="p-5">
              ...
              {activeModal === 'match-confirmation' && matchOrder && matchDriver && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono block">Dados da Carga</span>
                    <div className="bg-zinc-50 border border-zinc-200 p-3 rounded-xl space-y-1">
                      <p className="text-xs font-bold text-zinc-900">{matchOrder.client} ({matchOrder.id})</p>
                      <p className="text-[11px] text-zinc-500 font-medium">{matchOrder.origin} → {matchOrder.destination}</p>
                      <p className="text-[11px] text-zinc-400 font-semibold pt-1">Peso: {matchOrder.weight} • Valor declarado: {matchOrder.value}</p>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono block">Operador Selecionado</span>
                    <div className="bg-zinc-50 border border-zinc-200 p-3 rounded-xl space-y-1">
                      <p className="text-xs font-bold text-zinc-900">{matchDriver.name}</p>
                      <p className="text-[11px] text-zinc-500 font-medium">{matchDriver.vehicle}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3 bg-zinc-50 border border-zinc-200 p-3 rounded-xl text-center">
                    <div>
                      <span className="text-[9px] font-bold text-zinc-400 block uppercase tracking-wider">Distância Deslocamento</span>
                      <span className="text-xs font-black text-zinc-900">{matchDriver.distance}</span>
                    </div>
                    <div>
                      <span className="text-[9px] font-bold text-zinc-400 block uppercase tracking-wider">Tempo até Coleta</span>
                      <span className="text-xs font-black text-zinc-900">{matchDriver.eta}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    ...
                    <button onClick={handleExecuteMatch} className="bg-zinc-950 text-amber-400 py-2.5 rounded-xl text-xs font-black hover:bg-amber-400 hover:text-zinc-950 transition-colors cursor-pointer">
                      Confirmar Match
                    </button>
                  </div>
                </div>
              ...
              {activeModal === 'driver-detail' && selectedDriver && (
                <div className="space-y-4">
                  ...
                  <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-3 space-y-1 text-xs text-zinc-600 font-medium">
                    <p><span className="text-zinc-400 font-bold">Último Ponto Identificado:</span> {selectedDriver.location}</p>
                    {selectedDriver.phone && <p><span className="text-zinc-400 font-bold">Contato Direto:</span> {selectedDriver.phone}</p>}
                  </div>
                  <button onClick={() => setActiveModal(null)} className="w-full bg-zinc-950 text-white py-2.5 rounded-xl text-xs font-bold cursor-pointer">
                    Fechar Janela
                  </button>
